...
3. If Tutor Kit files are missing, run `init` via the wrapper. Then run `begin <id> [title]`. This creates or resumes `tutor-work/<id>` and copies the current published source there when the textbook already exists. Capture the exact work area printed by the command.
...
7. Run `publish <id>` against the central library. Publish compiles the staged textbook, verifies coding problems, writes `compile-result.md`, archives the prior source, and replaces it as one operation. If verification fails, repair the retained work area and publish again.
8. Start the Tutor Kit app with the wrapper's `dev` command after creating the central learner library or when the user asks to study the material. Keep it running and report the localhost URL; do not restart after every edit.
