...
async function handleRequest(cwd, componentRegistry, request, response) {
    ...
    if (request.method === "GET" && textbookAssetMatch) {
        ...
        const assetPath = resolveTextbookAssetPath(workspace, decodeURIComponent(textbookAssetMatch[1] ?? ""), decodeURIComponent(textbookAssetMatch[2] ?? ""));
        if (!assetPath) {
    ...
    if (request.method === "GET" && url.pathname === "/api/textbooks") {
        ...
        const entries = loaded.textbooks.map(({ textbook }) => {
    ...
    if (request.method === "GET" && chapterMatch) {
        ...
        const found = loaded.chapters.find(({ textbookId: candidateTextbookId, chapter }) => (candidateTextbookId === textbookId && chapter.id === chapterId));
        if (!found) {
...
async function loadWithComponents(cwd, registry, options = {}) {
...
function firstIssueByTextbook(issues) {
...
function isKnownAppPath(pathname) {
    ...
    if (parts.length === 3 && parts[0] === "textbooks" && Boolean(parts[1]) && parts[2] === "glossary")
        return true;
    if (parts.length === 4 && parts[0] === "textbooks" && Boolean(parts[1]) && parts[2] === "glossary" && parts[3] === "study")
        return true;
