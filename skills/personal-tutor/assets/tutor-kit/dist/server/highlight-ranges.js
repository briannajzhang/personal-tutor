export function normalizeHighlightRange(startOffset, endOffset) {
    return {
        startOffset: Math.min(startOffset, endOffset),
        endOffset: Math.max(startOffset, endOffset)
    };
}
export function buildHighlightBlockSelections(inputs) {
...
export function selectionContainsUnsupportedText(parts) {
    return parts.some((part) => !part.supported && part.text.trim().length > 0);
}
export function selectionHasUnsupportedText(selectedText, supportedTexts) {
    let cursor = 0;
    for (const supportedText of supportedTexts) {
        ...
        if (selectedText.slice(cursor, index).trim())
            return true;
...
export function rangesTouchOrOverlap(left, right) {
    return left.startOffset <= right.endOffset && right.startOffset <= left.endOffset;
}
export function classifyHighlightSelection(selection, ranges) {
...
export function touchedHighlightRanges(selection, ranges) {
    return ranges.filter((range) => rangesTouchOrOverlap(selection, range));
}
export function splitHighlightRange(highlight, removal) {
    const pieces = [];
...
function selectionFullyCovered(selection, ranges) {
    ...
    for (const range of sorted) {
        ...
        if (coveredUntil >= selection.endOffset)
            return true;
    }
