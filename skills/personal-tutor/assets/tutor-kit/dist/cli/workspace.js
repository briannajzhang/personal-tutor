...
export function addBlock(cwd, kind) {
    const result = { created: [], skipped: [] };
    ensureDir(join(cwd, "tutor", "blocks"));
    if (["p", "heading", "list", "codeBlock", "mathBlock", "callout", "transformation", "glossary", "quiz", "codingProblem", "core"].includes(kind)) {
        writeIfMissing(join(cwd, "tutor", "blocks", "core.tsx"), coreBlocksTemplate(), result);
        return result;
    }
    throw new Error(`Unknown block "${kind}". Available blocks: p, heading, list, codeBlock, mathBlock, callout, transformation, glossary, quiz, codingProblem.`);
}
