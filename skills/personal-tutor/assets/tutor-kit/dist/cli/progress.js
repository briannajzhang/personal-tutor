...
export function formatProgress(summary) {
    ...
    if (summary.openHighlights > 0)
        lines.push(`open highlights: ${summary.openHighlights}`);
    if (summary.invalidEventCount > 0)
        lines.push(`ignored invalid events: ${summary.invalidEventCount}`);
...
function summarizeQuizzes(events) {
    const summaries = new Map();
    for (const event of events) {
        if (event.type !== "quiz_checked")
            continue;
        ...
        if (!textbookId || !chapterId || !quizId || score === null || total === null)
            continue;
        ...
        for (const response of responseRecords(event.responses)) {
            ...
            if (questionId)
                current.missed.set(questionId, (current.missed.get(questionId) ?? 0) + 1);
        }
...
function summarizeWeakTags(events, questionTags) {
    ...
    for (const event of events) {
        ...
        const chapterId = text(event.chapterId);
        const quizId = text(event.quizId);
        if (!textbookId || !chapterId || !quizId)
            continue;
        for (const response of responseRecords(event.responses)) {
            if (response.correct === true)
                continue;
...
function summarizeCoding(events) {
    const summaries = new Map();
    for (const event of events) {
        if (event.type !== "coding_action_ran")
            continue;
        const textbookId = text(event.textbookId);
        const chapterId = text(event.chapterId);
        const blockId = text(event.blockId);
        if (!textbookId || !chapterId || !blockId)
            continue;
...
function summarizeGlossary(events) {
    const again = new Map();
    for (const event of events) {
        if (event.type !== "glossary_card_rated" || event.rating !== "again")
            continue;
        const termId = text(event.termId);
        if (termId)
            again.set(termId, (again.get(termId) ?? 0) + 1);
    }
...
function countOpenHighlights(events) {
...
function questionKey(textbookId, chapterId, quizId, questionId) {
