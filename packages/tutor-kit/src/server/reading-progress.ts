...
export interface ReadingProgressSummary {
  ...
  percent: number;
  lastChapter: { id: string; title: string; headingId: string | null } | null;
  continueChapter: { id: string; title: string; headingId: string | null } | null;
  completedChapterIds: string[];
}
...
export function loadReadingProgress(dataDir: string, textbookId: string): ReadingProgressState {
  const path = readingProgressPath(dataDir, textbookId);
  const parsed = readJsonFile<Partial<ReadingProgressState>>(path);
  if (!parsed) return emptyReadingProgress();
  return {
...
export function summarizeReadingProgress(state: ReadingProgressState, textbook: Textbook): ReadingProgressSummary {
...
export async function updateReadingProgress(
...
): Promise<{ summary: ReadingProgressSummary }> {
  ...
  if (loaded.issues.length > 0) throw new Error(loaded.issues.map((issue) => issue.message).join("\n"));
  const textbook = loaded.textbooks[0]?.textbook;
  if (!textbook) throw new Error(`Textbook not found: ${textbookId}`);
  ...
  if (action === "reopen") completed.delete(chapterId);
...
function emptyReadingProgress(): ReadingProgressState {
  return {
    lastChapterId: null,
    lastHeadingId: null,
    visitedChapterIds: [],
...
function readingProgressPath(dataDir: string, textbookId: string): string {
  return join(dataDir, "reading-progress", `${strictSegment(textbookId, "textbookId")}.json`);
}

function strictSegment(value: string, label: string): string {
...
function readingAction(value: unknown): "visit" | "complete" | "reopen" {
  if (value === "visit" || value === "complete" || value === "reopen") return value;
  throw new Error("action must be visit, complete, or reopen");
}

function chapterHeadingIds(chapter: Textbook["chapters"][number]): Set<string> {
