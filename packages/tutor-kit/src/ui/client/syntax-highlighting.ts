export const plainSyntaxLanguages = ["text", "txt", "plain", "plaintext"] as const;

export const syntaxLanguageAliases: Record<string, string> = {
  bash: "shellscript",
  js: "javascript",
  jsx: "jsx",
  md: "markdown",
...
const plainLanguageSet = new Set<string>(plainSyntaxLanguages);

export function normalizeSyntaxLanguage(value: unknown): string | null {
  const language = String(value ?? "").trim().toLowerCase();
  if (!language || plainLanguageSet.has(language)) return null;
  return syntaxLanguageAliases[language] ?? language;
}

export function syntaxHighlightingClientJs(): string {
  return `
