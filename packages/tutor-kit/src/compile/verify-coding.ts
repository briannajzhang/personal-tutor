...
async function verifyProblem(
  ...
  if (!verification) return { ok: false, message: "missing verification metadata" };
  const action = problem.props.actions.find((candidate) => candidate.id === verification.actionId);
  if (!action) return { ok: false, message: `verification action not found: ${verification.actionId}` };

  const starter = await runVariant(problem, action.command, {}, config);
  if (starter.setupFailed) return { ok: false, message: `starter setup/runtime failed: ${formatShell(starter.result)}` };
  if (starter.result.timedOut) return { ok: false, message: "starter verification timed out" };
  if (starter.result.exitCode === 0) return { ok: false, message: "starter unexpectedly passed" };
  ...
  if (reference.setupFailed) return { ok: false, message: `reference setup/runtime failed: ${formatShell(reference.result)}` };
  if (reference.result.timedOut) return { ok: false, message: "reference verification timed out" };
  if (reference.result.exitCode !== 0) return { ok: false, message: `reference solution failed: ${formatShell(reference.result)}` };
